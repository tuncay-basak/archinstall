#!/bin/bash
# ============================================================================
# Interactive Arch Linux Installer – clear, safe, step‑by‑step
# ============================================================================
# No arguments. Guides through:
#   - Disk selection (with full disk wipe option)
#   - Root partition (existing partition or new one in free space)
#   - EFI partition detection / creation
#   - Filesystem (ext4/btrfs/xfs) and LUKS encryption (optional)
#   - Hostname, root password, sudo user
...
# Helper: yes/no confirmation (default no)
confirm() {
    local prompt="$1 (y/N): "
    local answer
...
# Helper: selection from a list with detailed options
select_from_list() {
    local title="$1"
    shift
    local options=("$@")
